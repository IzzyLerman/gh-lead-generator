...
BEGIN
  ...
  IF queue_size >= queue_max THEN
    worker_url := (select decrypted_secret from vault.decrypted_secrets where name = 'generate_message_url');
    ...
    IF worker_url IS NOT NULL THEN
      insert into public.debug_logs (message) VALUES ('[trigger_email_generation()] Making HTTP POST to: ' || worker_url);
