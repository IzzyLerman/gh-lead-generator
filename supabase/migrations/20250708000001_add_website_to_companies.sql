...
ALTER TABLE public.companies ADD COLUMN website TEXT DEFAULT '';

-- Create normalize_website function
CREATE OR REPLACE FUNCTION private.normalize_website(website_input TEXT)
RETURNS TEXT AS $$
BEGIN
    RETURN LOWER(TRIM(website_input));
END;
$$ LANGUAGE plpgsql IMMUTABLE SECURITY DEFINER SET search_path = private;

-- Update upsert_company function to include website parameter
DROP FUNCTION private.upsert_company(p_name TEXT, p_email TEXT, p_phone TEXT, p_industry TEXT[], p_city TEXT, p_state TEXT);
